...
def main():
    ...
    try:
        ...
        print(f"✅ Server is reachable (status={status or 'unknown'}).")
    ...
    if len(online_clouds) < 3:
        print(
            "⚠️ Warning: Less than 3 cloud instances found, will reuse them or only run partial tests."
        )
    ...
    for i, prompt in enumerate(flows):
        ...
        payload = {
            ...
            "payload": {
                ...
                "allowed_actions": ["ui.click", "ui.input_text", "ui.swipe", "ui.key_press"],
            },
            "targets": [target],
            "priority": 50,
        }
        ...
        try:
            print(
                f"Submitting flow {i + 1}: {prompt} to Cloud {target['cloud_id']} on Device {target['device_id']}"
            )
    ...
    print("\n--- Summary ---")
    print(
        "All tasks submitted. You can monitor the progress on the web console (http://127.0.0.1:8001/web)"
    )
