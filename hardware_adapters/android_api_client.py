...
class AndroidApiClient:
    ...
    def __init__(self, device_ip: str, api_port: int, timeout_seconds: float = 30.0, retries: int = 3) -> None:
        self.http = BaseHTTPClient(device_ip, api_port, timeout_seconds=timeout_seconds, retries=retries)
    ...
    def set_s5_proxy(self, ip: str, port: int, username: str, password: str, proxy_type: int = 2) -> Dict[str, Any]:
        """GET /proxy?cmd=2&..."""
        invalid = self._require_non_empty(ip, "ip")
        if invalid:
            return invalid
        return self.http.get("/proxy", query={"cmd": 2, "type": proxy_type, "ip": ip, "port": int(port), "usr": username, "pwd": password})
    ...
    def screenshot(self, image_type: int = 0, quality: int = 80, save_path: str = "") -> Dict[str, Any]:
        """GET /snapshot"""
        result = self.http.request_bytes("GET", "/snapshot", query={"type": image_type, "quality": quality})
    ...
    def upload_file(self, local_path: str = "", remote_path: str = "", file_url: str = "") -> Dict[str, Any]:
        ...
        return self.http.post_multipart("/upload", fields=fields, files={"file": (f.name, f.read_bytes(), mime)})
    ...
    def install_apks(self, apk_paths: list[str] | None = None, zip_path: str = "") -> Dict[str, Any]:
        ...
        if zip_path.strip():
            ...
            return self.http.post_multipart("/installapks", files={"file": (f.name, f.read_bytes(), mime)})
        if not apk_paths:
    ...
    def restore_app(self, backup_path: str) -> Dict[str, Any]:
        ...
        if invalid:
            return invalid
        return self.http.get("/backrestore", query={"cmd": "recovery", "backuppath": backup_path, "path": backup_path})
    ...
    def set_root_allowed_app(self, package: str, allowed: bool) -> Dict[str, Any]:
        ...
        if invalid:
            return invalid
        return self.http.get("/modifydev", query={"cmd": 10, "pkg": package, "root": "true" if allowed else "false"})

    def get_boot_apps(self) -> Dict[str, Any]:
        """GET /appbootstart?cmd=1"""
        return self.http.get("/appbootstart", query={"cmd": 1})

    def set_boot_app(self, package: str, enabled: bool = True, packages: list[str] | None = None) -> Dict[str, Any]:
        ...
        return self.http.request_json("POST", "/appbootstart", payload=payload, query={"cmd": 2 if enabled else 3})

    # --- 联系人 ---

    def add_contact(self, name: str = "", number: str = "", contacts: list[dict[str, str]] | None = None) -> Dict[str, Any]:
        """GET /addcontact?data=[]"""
        if contacts is None:
    ...
    def set_background_keepalive(self, package: str, enabled: bool = True, cmd: int | None = None) -> Dict[str, Any]:
    ...
    def set_virtual_camera(self, path: str = "", type: str = "", resolution: str = "") -> Dict[str, Any]:
        """GET /modifydev?cmd=4"""
    ...
    def module_manager(self, cmd: str, module: str) -> Dict[str, Any]:
        ...
        return self.http.get("/modulemgr", query={"cmd": cmd_value, "module": module_value, "moduler": module_value})
    ...
    def upload_google_cert(self, cert_path: str) -> Dict[str, Any]:
        ...
        return self.http.post_multipart("/uploadkeybox", files={"file": (f.name, f.read_bytes(), mime)})
