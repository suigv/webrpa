...
def android_set_proxy(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    username = str(params.get("s5User") or params.get("username") or params.get("usr") or "").strip()
    password = str(params.get("s5Password") or params.get("password") or params.get("pwd") or "").strip()
    ...
    return _from_api(client.set_s5_proxy(ip=ip, port=int(port), username=username, password=password, proxy_type=proxy_type))
...
def android_upload_file(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    if client is None:
        return _err("invalid_params", "device_ip is required")
    return _from_api(client.upload_file(
...
def android_receive_sms(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    if client is None:
        return _err("invalid_params", "device_ip is required")
    return _from_api(client.receive_sms(
...
def android_set_virtual_camera_source(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    return _from_api(client.set_virtual_camera(
...
def android_get_app_bootstart_list(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
...
def android_get_webrtc_player_url(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    return _from_api(client.get_webrtc_player_url(
...
def android_set_background_keepalive(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    cmd = params.get("cmd")
...
def android_query_background_keepalive(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
...
def android_add_background_keepalive(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    package = str(params.get("package") or "").strip()
...
def android_remove_background_keepalive(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    package = str(params.get("package") or "").strip()
...
def android_update_background_keepalive(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
    package = str(params.get("package") or "").strip()
...
def android_import_app_info(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    if client is None:
        return _err("invalid_params", "device_ip is required")
    package = str(params.get("package") or "").strip()
    data_path = str(params.get("data_path") or params.get("backuppath") or params.get("backup_path") or "").strip()
    if not data_path:
...
def android_camera_hot_start(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    if client is None:
        return _err("invalid_params", "device_ip is required")
    return _from_api(client.camera_hot_start(
        enabled=bool(params.get("enabled", True)),
        path=str(params.get("path") or ""),
    ))
...
def android_autoclick(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    if action:
        return _from_api(client.autoclick_action(
            action=action,
            x=params.get("x"),
            y=params.get("y"),
    ...
    return _from_api(client.autoclick(
        enabled=bool(params.get("enabled", True)),
        interval_ms=params.get("interval_ms"),
    ))


def android_get_root_allowed_apps(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    client = _api_client(params, context)
    if client is None:
        return _err("invalid_params", "device_ip is required")
