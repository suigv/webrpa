...
def screenshot(params: dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        if method is None:
            return ActionResult(
                ok=False, code="not_supported", message="screentshot is not available"
            )
...
def capture_raw(params: dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        if all(v is not None for v in (left, top, right, bottom)):
            payload = (
                rpc.take_capture_ex(_to_int(left), _to_int(top), _to_int(right), _to_int(bottom))
                if rpc is not None
                else None
            )
        else:
...
def capture_compressed(params: dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        if payload is None:
            return ActionResult(
                ok=False, code="capture_failed", message="compressed capture failed"
            )
        if save_path:
            with open(save_path, "wb") as f:
                f.write(payload)
        return ActionResult(
            ok=True, code="ok", data={"byte_length": len(payload), "save_path": save_path or None}
        )
    finally:
        _close_rpc(rpc)
...
def check_connect_state(params: dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            ok=connected, code="ok" if connected else "not_connected", data={"connected": connected}
        )
    finally:
        _close_rpc(rpc)
...
def use_new_node_mode(params: dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(
            ok=ok, code="ok" if ok else "set_mode_failed", data={"enabled": enabled}
        )
    finally:
        _close_rpc(rpc)
