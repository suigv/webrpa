...
def create_selector(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    return _selector_support.create_selector(params, context, get_rpc=_get_rpc, close_rpc=_close_rpc)
...
def selector_free(params: Dict[str, Any], context: ExecutionContext, *, close_rpc: Any = _close_rpc) -> ActionResult :
    """Legacy alias for selector_clear."""
    return _selector_support.selector_clear(params, context, close_rpc=close_rpc)

def selector_clear(params: Dict[str, Any], context: ExecutionContext, *, close_rpc: Any = _close_rpc) -> ActionResult:
    return _selector_support.selector_clear(params, context, close_rpc=close_rpc)

def release_selector_context(context: ExecutionContext, *, close_rpc: Any = _close_rpc) -> ActionResult:
...
def dump_node_xml_ex(params: Dict[str, Any], context: ExecutionContext) -> ActionResult:
    ...
    try:
        ...
        return ActionResult(ok=xml is not None, code="ok" if xml is not None else "dump_failed", data={"xml": xml})
    finally:
        _close_rpc(rpc)
