...
def test_coerce_point_scaling():
    ...
    # 1. Pixel 模式 (不缩放)
    x, y = ai_actions._coerce_point(
        100, 200, width=width, height=height, coord_mode="pixel", clamp=True
    )
    assert x == 100
    assert y == 200
    ...
    # 500/1000 * 1920 = 960
    x, y = ai_actions._coerce_point(
        500, 500, width=width, height=height, coord_mode="norm_1000", clamp=True
    )
    assert x == 540
    assert y == 960
    ...
    # 3. Normalized 模式 (0-1)
    x, y = ai_actions._coerce_point(
        0.5, 0.5, width=width, height=height, coord_mode="norm_1", clamp=True
    )
    assert x == 540
    assert y == 960
...
def test_locate_point_with_physical_resolution():
    ...
    with patch("engine.actions.ai_actions.capture_compressed") as mock_capture:
        ...
        mock_response = MagicMock(
            ok=True, output_text='{"x": 500, "y": 500}', model="test", error=None
        )
        ...
        with patch("engine.actions.ai_actions.LLMClient", return_value=mock_client_instance):
            ...
            # y = (480 / 960) * 1920 = 960
            res = ai_actions.locate_point(
                {"prompt": "test", "coord_mode": "norm_1000"}, mock_context
            )
...
def test_locate_point_pixel_mode_preserves_accuracy():
    ...
    with patch("engine.actions.ai_actions.capture_compressed") as mock_capture:
        ...
        mock_response = MagicMock(
            ok=True, output_text='{"x": 270, "y": 800}', model="test", error=None
        )
        mock_client_instance.evaluate.return_value = mock_response
...
def test_locate_point_landscape_orientation():
    ...
    with patch("engine.actions.ai_actions.capture_compressed") as mock_capture:
        ...
        mock_response = MagicMock(
            ok=True, output_text='{"x": 480, "y": 500}', model="test", error=None
        )
        mock_client_instance.evaluate.return_value = mock_response
...
def test_locate_point_accepts_instruction_alias():
    ...
    with patch("engine.actions.ai_actions.capture_compressed") as mock_capture:
        ...
        mock_client_instance = MagicMock()
        mock_response = MagicMock(
            ok=True, output_text='{"x": 10, "y": 20}', model="test", error=None
        )
        mock_client_instance.evaluate.return_value = mock_response
