...
from ai_services.llm_client import LLMClient, LLMRequest, ResolvedLLMRequest
...
def test_resolve_request_with_default_provider(mock_settings):
    with patch("core.system_settings_loader.load", return_value=mock_settings):
...
def test_runtime_config_override(mock_settings):
    with patch("core.system_settings_loader.load", return_value=mock_settings):
        with patch.dict(os.environ, {"MYT_LLM_API_KEY": "key"}, clear=True):
            client = LLMClient()
            runtime_config = {
                "llm": {
                    "provider": "openai",
