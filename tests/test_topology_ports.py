...
def test_topology_unique_ports():
    ...
    try:
        ...
        assert len(port_pairs) == clouds, (
            f"Expected {clouds} unique port pairs, got {len(port_pairs)}"
        )
        ...
        for cloud_id, (expected_api, expected_rpa) in expected.items():
            ...
            for device_id in range(1, total_devices + 1):
                ...
                assert api_port == expected_api, (
                    f"cloud {cloud_id}: expected {expected_api}, got {api_port}"
                )
                assert rpa_port == expected_rpa, (
                    f"cloud {cloud_id}: expected {expected_rpa}, got {rpa_port}"
                )
    finally:
        ConfigLoader._config = backup
...
def test_allocator_depends_on_cloud_index_not_device_index():
    ...
    try:
        ConfigLoader._config = {"sdk_port": 8000}
        assert calculate_ports(device_index=1, cloud_index=3, cloud_machines_per_device=12) == (
            30201,
            30202,
        )
        assert calculate_ports(device_index=9, cloud_index=3, cloud_machines_per_device=12) == (
            30201,
            30202,
        )
    finally:
