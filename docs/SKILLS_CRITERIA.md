...
### 2.3 复合技能 (Composite Skills / Plugins) - **终极演进形态**
由多个原子动作组合而成的业务逻辑，是 **“行为编译器”** 的最终产物。
- **例子**：`x.login` (包含 `app.open` + `ui.input_text` + `ui.click`)。
- **治理**：这是成熟度漏斗的 **Master 阶段**，由 AI 探索并自动蒸馏生成的 YAML 插件，代表了业务逻辑的工程化最高标准。

## 3. 治理执行路线

1. **Tagging (打标)**：在 `ActionMetadata` 中引入 `tags` 字段。
2. **Discovery Filter (过滤发现)**：
    - `/api/engine/schema` 作为通用发现目录，默认返回完整 metadata，并支持 `tag` 过滤。
    - `/api/engine/skills` 作为 AI Agent 的“技能书”，仅包含 `tags=["skill"]` 的条目。
3. **Promotion (晋升机制)**：
