...
> 说明：本文件同时包含“当前能力摘要”和“按日期追加的历史变更日志”。涉及当前契约、当前里程碑状态与当前接口语义时，以 `docs/README.md`、`docs/STATUS.md`、`docs/strategy/ROADMAP.md`、`docs/HTTP_API.md`、`docs/PLUGIN_CONTRACT.md` 为准；下方较早日期的条目应按历史记录理解，不应直接视为当前实现清单。
...
- 最近重点 (2026-03-17):
  ...
  - **Inventory / Selector / Generator 初始化能力落地 (2026-03-19)**：
    ...
    - **冗余与回退治理 (2026-03-19)**：
      ...
      - **后端前端兼容点审查**：旧的 `/api/tasks/distill/{plugin_name}` 插件蒸馏路由已删除，当前仍保留的主要兼容入口集中在 runtime target 缺少 `device_ip` 时回退读取 payload 的兜底逻辑等少量执行层兼容口。
      ...
      - **插件蒸馏返回骨架收口**：`task_routes.py` 中“不支持蒸馏”和“未达到门槛”这两段返回骨架保持为共享 helper，当前统一只由 `/api/tasks/plugins/{plugin_name}/distill` 主入口调用。
...
- 最近重点 (2026-03-15)：
  ...
  - **API 边界加固**：
    - `/api/tasks/plugins/{plugin_name}/distill`：加入 `plugin_name` 严格校验与输出目录边界约束（仅允许写入 `plugins/` 下），并将蒸馏门槛从插件 `manifest.yaml` 的 `distill_threshold` 读取，路由层不再硬编码。
...
- 最近重点 (2026-03-12)：
  ...
  - **多轮成功率统计与蒸馏触发**：
    ...
    - 新增 `POST /api/tasks/plugins/{plugin}/distill`，一键触发多轮蒸馏生成 YAML 草稿。
...
- **文档与系统一致性对齐**：全量审计并修复了 `docs/` 下的过期信息，包括多云机动态端口公式、任务控制面架构拆分描述、以及插件契约规范（补全 Pydantic 必填字段），并新增了 **[Skills化演进报告](../strategy/SKILLS_EVOLUTION.md)**，确立了 AI 驱动技能化的架构演进方向。
...
- 最近重点 (本会话)：
  - **前后端蒸馏入口对齐**：
    - `api/routes/task_routes.py` 现在统一只保留插件蒸馏主入口 `/api/tasks/plugins/{plugin_name}/distill`。
    - 指标页蒸馏按钮已切换到该入口，避免前端继续把 legacy route 当成主契约。
    - 补充主入口对不可蒸馏插件与门槛不足场景的定向测试，锁住当前主契约。
