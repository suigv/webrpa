...
- **插件目录**：`plugins/`
- **默认提示词**：`engine/prompt_templates.py`（单一默认模板数据源，`GET /api/tasks/prompt_templates` 路由对外暴露，前端打开 AI 对话框时自动拉取并填充）
- **UI 配置**：`config/apps/*.yaml`（默认 `config/apps/default.yaml`，可通过 `MYT_DEFAULT_APP` 指定回退配置）
- **UI 状态配置**：`config/apps/*.yaml`（按 `package_name` 动态加载，`xml_filter`/`states`/`selectors` 统一存放；缺失时回退到通用观察链路）
