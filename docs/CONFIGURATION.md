...
- 只有在 `discovery_enabled=true` 时，运行时才会采用局域网发现结果；禁用状态下始终使用静态 `host_ip` / `device_ips`。
- 读取配置、设备列表或拓扑时不会隐式触发局域网扫描；扫描只应由显式启用 discovery 或调用手动发现接口触发。
- discovery 判活会校验 8000 端口上的 MYT SDK `/info` 或 `/info/device` 响应形状，而不是仅凭 TCP 端口可连接就把该主机记为云机。
