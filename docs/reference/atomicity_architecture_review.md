...
- `engine/actions/ui_actions.py`、`engine/actions/sdk_actions.py`、`core/task_control.py` 已经跨过舒适维护区间，开始表现出 God module / orchestration leakage 特征。
...
- `core/task_control.py` 已经同时承担提交、调度、失败恢复、事件发射、重试策略、目标分发，以及一部分业务反馈逻辑。

问题：

- `TaskController` 正在从 orchestration service 演化成总控制器。
- 失败反馈一类业务策略进入调度核心后，会持续侵蚀模块边界。
- 重试、事件、分发、反馈混在一个类中，会让后续测试和修改成本同时上升。

判断：

- Core 的三分结构是对的。
- 真正的风险集中在 `TaskController` 的职责扩张，而不是 store/queue/events 的拆分方向。

## 当前最不合理的设计点

按优先级排序：

1. `core/task_control.py` 中存在 orchestration 之外的业务策略钩子
2. `engine/actions/ui_actions.py` 将 RPC 基础能力与 selector/node 子系统塞在同一模块
3. `engine/actions/sdk_actions.py` 将 SDK 绑定、参数装配、共享状态和策略逻辑混在一起
4. `engine/actions/state_actions.py` 与 `engine/actions/ui_actions.py` 存在重复 RPC 接入模式
5. `plugins/x_mobile_login/script.yaml` 作为单插件工作流已经过长，且重复模式太多
...
- 将 `core/task_control.py` 中的业务反馈逻辑抽到独立 hook/service
...
1. 先做一次低风险整理：提取共享 RPC helper，抽出 `TaskController` 的业务反馈 hook
