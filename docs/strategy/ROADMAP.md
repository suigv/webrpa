...
This roadmap translates `docs/strategy/PROJECT_GOALS.md` into milestones with clear completion criteria and a status signal that distinguishes verified work from unverified implementation.
...
- Add a backend-owned device WebRTC URL/ticket API so the frontend does not hardcode device IP/port formulas.
- Extend port calculation with documented WebRTC TCP/UDP ports (`+7` / `+8`) and keep the existing API (`+1`) / RPC (`+2`) roles unchanged.
- Ensure `web/webplayer` assets are published in the real frontend deployment path; current Vite build does not emit them into `web/dist/`.
- Validate operator-browser reachability to device WebRTC endpoints, plus HTTPS/WSS, auth, and audit boundaries before enabling by default.

## Evidence Anchors (Where to Attach Proof)
- Validation gates: `.sisyphus/evidence/` and `docs/governance/project_progress.md`.
