...
test('AI 工作台在引导模式和高级模式间共享同一套任务图状态', async ({ page }) => {
  let lastPlannerRequest: Record<string, unknown> | null = null;
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/ai_dialog/planner' && method === 'POST') {
      lastPlannerRequest = route.request().postDataJSON() as Record<string, unknown>;
      return json({
        display_name: 'X 登录规划',
  ...
  await page.locator('#aiWorkspaceNoAccountRequired').check();
  await expect(page.locator('#aiWorkspaceAccountSelect')).toBeDisabled();
  await expect(page.locator('#aiWorkspaceAccountHint')).toContainText('当前任务已声明无需账号数据');
  ...
  expect(lastPlannerRequest?.account_required).toBe(false);
  ...
  await expect(page.locator('#aiWorkspaceNoAccountRequired')).toBeChecked();
  await expect(page.locator('#aiWorkspaceGraphSummary')).toContainText('当前任务图已确认');
